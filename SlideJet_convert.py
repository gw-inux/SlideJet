...
if uploaded_file:
    ...
    if deployment_mode == "Online use (Streamlit Cloud)":
        yaml_repo_path = st.text_input(
            "Enter the relative path from the repository to the YAML file (e.g., `project\presentations`). The name of the repository is usually not part of the path",
            value=""
        )
    st.markdown("""
    #### Relative path to the presentation data
    :orange[**SlideJet-Present**] access the data from a folder that is typically named **slides**. Within slides, each presentation can be saved in a subfolder. Subsequently, you can define the relative path, whereas the presetting is likely suitable for most users:
    """
    )
    # Slides folder (relative to SlideJet_present folder)
    slides_subfolder = st.text_input(
        "Enter the relative path where the slides will be saved (from YAML location). Usually it 'slides/[NAME_OF_PRESENTATION]'",
    ...
    if start_convert:
        ...
        if slide_data:
            ...
            st.markdown("""
            #### Next steps
            Now you will find the slides, the speaker notes (as *.json file), and the YAML file in the generated folders - see messages above. The see and present the slides, use the :orange[**SlideJet_present.py**] application with the generated YAML file. You have two options:
            (1) Directly refer to the YAML file from the :orange[**SlideJet_present.py**] application, or
            (2) Select and load the YAML file from the :orange[**SlideJet_present.py**] application.
            """)
